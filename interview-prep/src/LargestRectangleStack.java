import java.util.*;

/**
 * Problem: https://www.hackerrank.com/challenges/largest-rectangle/problem
 *
 * ToDo: Improve runtime efficiency
 */
public class LargestRectangleStack {

    public static void main(String[] args){
        int n = 5;
        int[] adjBuildings = {6873,7005,1372,5438,1323,9238,9184,2396,4605,162,7170,9421,4012,5302,6277,2438,4409,3391,4956,4488,622,9365,5088,6926,2691,6909,1050,2824,3538,5801,8468,411,9158,9841,2201,481,5431,1385,2877,36,1547,48,5809,1911,1702,8439,4349,6111,1830,5657,6951,8804,5022,8391,2083,7713,5300,3133,6890,5190,5286,1710,1953,4445,7903,4154,4926,3335,5539,4156,9723,3438,556,1885,5349,2258,324,6050,4722,8506,1707,1673,7310,3081,65,9393,7147,1717,8878,389,6908,4165,2099,5213,8610,3,9368,3536,9690,1259};
        List<Integer> areas = new ArrayList<>();
        int result = largestRectangle(adjBuildings, areas);
        System.out.format("For %d adjacent buildings as described by;\n %s\n, the largest area is: %d",
                n, Arrays.toString(adjBuildings),result);

    }

    public static int largestRectangle(int[] adjBuildings, List<Integer> areas) {

        if(adjBuildings.length == 0){
            return -1; //base-case when array is empty
        }

        int[] minimumAndIndex = findMinimum(adjBuildings);
        System.out.println("Minimum: " + minimumAndIndex[0] + " @ index: " + minimumAndIndex[1]);
        System.out.println(Arrays.toString(adjBuildings));
        int area = adjBuildings.length * minimumAndIndex[0];
        System.out.println("Area: " + area);
        int[] subArrayLeft = new int[minimumAndIndex[1]];
        int[] subArrayRight = new int[adjBuildings.length - (minimumAndIndex[1]+1)];
        for(int i=0; i<subArrayLeft.length; i++){
           subArrayLeft[i] = adjBuildings[i];
        }
        for(int i=0; i<subArrayRight.length; i++){
            subArrayRight[i] = adjBuildings[minimumAndIndex[1]+1+i];
        }
       largestRectangle(subArrayLeft, areas);
       largestRectangle(subArrayRight, areas);
       areas.add(area);
       return Collections.max(areas);
    }

    public static int[] findMinimum(int[] adjBuildings){
        int min = adjBuildings[0];
        int idx = 0;
        for(int i=0; i<adjBuildings.length; i++){
            if(adjBuildings[i]<min){
                min = adjBuildings[i];
                idx = i;
            }
        }
        return new int[]{min, idx};
    }
}
